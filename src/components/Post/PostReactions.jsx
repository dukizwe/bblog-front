...
export default function PostReactions({postId, reactions: initialReactions}) {
          ...
          const likePost = async (e) => {
                    ...
                    if(!user) return
                    setLoading(true)
                    const reactions = await fetchApi(`/posts/${postId}/likes`, {
                              method: 'PUT'
                    })
                    setLoading(false)
          ...
          const dislikePost = async (e) => {
                    ...
                    if(!user) return
                    setLoading(true)
                    const reactions = await fetchApi(`/posts/${postId}/dislikes`, {
                              method: 'PUT'
                    })
                    setLoading(false)
                    setReactions(reactions)
          ...
          const LikeButton = () => {
                    ...
                    return <div className="likes__count">
                              <button onClick={likePost} className={likedCls} disabled={loading}>
                                        <LikeIcon />
          ...
          const DislikeButton = () => {
                    const DiskikeIcon = () => {
                              return reactions.vote === -1 ? 
                                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-hand-thumbs-down-fill" viewBox="0 0 16 16">
                    ...
                    return <div className="dislikes__count">
                              <button onClick={dislikePost} className={dislikedCls} disabled={loading}>
                                        <DiskikeIcon />
